...
var (
	...
	userBasicRowsExpectAutoSet   = strings.Join(stringx.Remove(userBasicFieldNames, "`id`", "`update_at`", "`updated_at`", "`update_time`", "`create_at`", "`created_at`", "`create_time`"), ",")
	userBasicRowsWithPlaceHolder = strings.Join(stringx.Remove(userBasicFieldNames, "`id`", "`update_at`", "`updated_at`", "`update_time`", "`create_at`", "`created_at`", "`create_time`"), "=?,") + "=?"
...
func (m *defaultUserBasicModel) Insert(ctx context.Context, data *UserBasic) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Identity, data.Name, data.Password, data.Email, data.NowVolume, data.TotalVolume, data.DeletedAt)
	}, userBasicIdKey)
	return ret, err
}
...
func (m *defaultUserBasicModel) Update(ctx context.Context, data *UserBasic) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Identity, data.Name, data.Password, data.Email, data.NowVolume, data.TotalVolume, data.DeletedAt, data.Id)
	}, userBasicIdKey)
	return err
}
